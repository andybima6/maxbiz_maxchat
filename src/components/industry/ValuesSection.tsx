...
const ValuesSection: React.FC<ValuesSectionProps> = ({
  ...
  return (
    <section className={`py-20 relative overflow-hidden ${gradientBg ? "bg-gradient-hero" : "bg-background"}`}>
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="text-center mb-16 text-primary-foreground">
          ...
          <AnimatedElement animation="fade-in" delay={400} duration={800}>
            <p className="text-xl text-primary-foreground/90 max-w-3xl mx-auto">{subtitle}</p>
          </AnimatedElement>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8 mb-12">
          {items.map((item, index) => (
            <AnimatedElement key={index} animation="fade-in" delay={index * 200} duration={800}>
              <Card className={`p-8 ${color.cardBg} backdrop-blur-sm hover:shadow-elegant transition-all duration-300 hover:-translate-y-2`}>
                ...
                <h3 className={`text-2xl font-bold mb-4 lg:min-h-[60px] ${color.text}`}>{item.title}</h3>
                <p className={`${color.muted} leading-relaxed lg:min-h-[120px]`}>{item.description}</p>
              </Card>
            </AnimatedElement>
          ))}
        </div>
        ...
        {stats && stats.length > 0 && (
          <div className="grid md:grid-cols-4 gap-8 mt-16">
            {stats.map((stat, i) => (
              <AnimatedElement key={i} animation="fade-in" delay={i * 200} duration={800}>
